...
2. **同一則訊息平行派出三個反方**（必須用 Agent 工具、subagent_type 分別為 `skeptic`、`red-team`、`simplifier`，一次三發不得串行）。
   三反方的 model 底線**以協議 §5 為唯一正本**（速記：預設明指 opus；主迴圈為 opus/fable 系時可繼承；裁決由主迴圈自做）。每個 agent 的 prompt = 待審包原文 + 該鏡頭的任務指示。逐條抗辯時每條發現固定耗 3 個子代理（三鏡頭），單輪平行子代理總數上限 6＝一輪最多 2 條發現；更多條就分批送審，批與批的結論分開回報、不合併。
